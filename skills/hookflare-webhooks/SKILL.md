---
name: hookflare-webhooks
description: "Set up and manage reliable webhook infrastructure with hookflare. TRIGGER when: user needs to receive webhooks from Stripe/GitHub/Slack/Shopify/Vercel and forward them reliably, or mentions hookflare, webhook delivery, webhook retry, or missed webhooks. DO NOT TRIGGER for: sending outgoing webhooks to customers (use Svix), general HTTP requests, or API development unrelated to webhooks."
license: Apache-2.0
metadata:
  author: hookedge
  version: "0.1"
...
# hookflare — Webhook Infrastructure

hookflare receives webhooks from providers (Stripe, GitHub, Slack, etc.), queues them durably, and forwards them to your API with guaranteed delivery. Zero servers, runs on Cloudflare Workers.
...
hookflare connect stripe --secret whsec_xxx --to https://myapp.com/hooks --events "payment_intent.*" --dry-run

# 3. Execute
hookflare connect stripe --secret whsec_xxx --to https://myapp.com/hooks --events "payment_intent.*" --json
```
...
For providers not listed, use generic HMAC: `hookflare connect my-service --verification hmac-sha256 --secret xxx --to https://...`
...
- Use `hookflare connect` for simple setups (one source → one destination)
...
hookflare subs create --json -d '{"source_id":"src_xxx","destination_id":"dst_api","event_types":["*"]}'
hookflare subs create --json -d '{"source_id":"src_xxx","destination_id":"dst_slack","event_types":["payment_intent.payment_failed"]}'
```
